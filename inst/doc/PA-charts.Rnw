...
\title{Overview of PerformanceAnalytics' Charts and Tables}


\author{Peter Carl and Brian Peterson}
...
via CRAN. \R~ users with connectivity can simply type: \code{install.packages(\char`\"{}PerformanceAnalytics\char`\"{})}.
...
\caption{First Lines of the managers.df Object }

\label{fig:First-Lines-of}

<<LoadData,echo=T>>=
#data(managers.df)
managers.df=read.csv("/home/peter/dev/R/managers.csv",row.names=1)
head(managers.df)
@
\end{figure}
...
you can see in Figure \ref{fig:First-Lines-of}, \code{managers.df}
...
from January 1996. Similar data could be constructed using \code{mymanagers.df=read.csv(\char`\"{}/path/to/file/mymanagers.csv\char`\"{},
...
With the data object in hand, we will group together columns of interest
...
dim(managers.df)
managers.length = dim(managers.df)[1]
...
frInception.rows = (length(managers.df[,1]) - length(managers.df[,1][!is.na(managers.df[,1])]) + 1):length(managers.df[,1])
...
which is said to emulate the Matlab colorset. The \code{dark8equal},
...
<<ShowPalates,echo=F,fig=T>>=
layout(rbind(c(1,2),c(3,4)))
chart.CumReturns(managers.df[,c(manager.col,peers.cols)],
lwd=2,legend.loc="topleft", main="default")
chart.CumReturns(managers.df[,c(manager.col,peers.cols)],lwd=2,
...
Similarly, there are a few sets of symbols for scatter charts . These
...
<<Graph1,echo=T,fig=T>>=
charts.PerformanceSummary(managers.df[,c(manager.col,indexes.cols)], colorset=rich6equal, lwd=2, ylog=TRUE)
@
...
t(table.Returns(managers.df[,c(manager.col,indexes.cols)]))
@
...
table.MonthlyReturns(managers.df[,c(manager.col,peers.cols)])
@
...
chart.Boxplot(managers.df[ trailing36.rows, c(manager.col, peers.cols, indexes.cols)], main = "Trailing 36-Month Returns")
@
...
layout(rbind(c(1,2),c(3,4)))
chart.Histogram(managers.df[,1,drop=F], main = "Plain", methods = NULL)
chart.Histogram(managers.df[,1,drop=F], main = "Density", breaks=40, 
methods = c("add.density", "add.normal"))
chart.Histogram(managers.df[,1,drop=F], main = "Skew and Kurt", methods = c
("add.centered", "add.rug"))
chart.Histogram(managers.df[,1,drop=F], main = "Risk Measures", methods = c
("add.risk"))
@
...
<<Graph3,echo=T,fig=T>>=
chart.RiskReturnScatter(managers.df[trailing36.rows,1:8], rf=.03/12, main = "Trailing 36-Month Performance", colorset=c("red", rep("black",5), "orange", "green"))
@
...
<<Graph5,echo=T,fig=T>>=
charts.RollingPerformance(managers.df[, c(manager.col, peers.cols, indexes.cols)], rf=.03/12, colorset = c("red", rep("darkgray",5), "orange", "green"), lwd = 2)
@
...
<<Graph6,echo=T,fig=T>>=
chart.RelativePerformance(managers.df[ , manager.col, drop = FALSE], managers.df[ , c(peers.cols, 7)], colorset = tim8equal[-1], lwd = 2, legend.loc = "topleft")
@
...
chart.RelativePerformance(managers.df[ , c(manager.col, peers.cols) ], managers.df[, 8, drop=F], colorset = rainbow8equal, lwd = 2, legend.loc = "topleft")
@
...
<<tableCAPM,echo=T>>=
table.CAPM(managers.df[trailing36.rows, c(manager.col, peers.cols)], managers.df[ trailing36.rows, 8, drop=FALSE], rf = managers.df[ trailing36.rows, Rf.col])
@
\end{figure}
...
#chart.MultiScatter(Ra=managers.df[,1,drop=F],Rb=managers.df[,8,drop=F],linear.fit=T,Rf=managers.df[,10],excess.returns=T)
@
...
charts.RollingRegression(managers.df[, c(manager.col, peers.cols), drop = FALSE], managers.df[, 8, drop = FALSE], rf = .03/12, colorset = redfocus, lwd = 2)
@
...
<<Graph9,echo=T,fig=T>>=
chart.RollingCorrelation(managers.df[,c(manager.col, peers.cols)], managers.df[, 8, drop = FALSE], colorset = tim8equal, lwd = 2, main = "12-Month Rolling Correlation")
@
...
<<tableCorr,echo=T>>=
table.Correlation(managers.df[, c(manager.col, peers.cols)], managers.df[, 8, drop = F])
@
\end{figure}
...
table.DownsideRisk(managers.df[,1:6],rf=.03/12)
@
...
table.Drawdowns(managers.df[,1,drop=F])
@
\end{figure}


The \code{\link{UpDownRatios}} function will give you some insight
