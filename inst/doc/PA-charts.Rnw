...
\maketitle
\begin{abstract}
In this vignette, we focus on the graphs and tables available in
\code{\link{PerformanceAnalytics}}, and comment about some other measures. We
develop the examples using data for six (hypothetical) managers, a peer index,
and an asset class index.
\end{abstract} 
...
<<tableCAPM,echo=T>>=
table.CAPM(managers.df[trailing36.rows, c(manager.col, peers.cols)],
managers.df[ trailing36.rows, 8, drop=FALSE],
rf = managers.df[ trailing36.rows, Rf.col])
@
\end{figure}
...
<<tableCorr,echo=T>>=
table.Correlation(managers.df[, c(manager.col, peers.cols)], managers.df[, 8,
