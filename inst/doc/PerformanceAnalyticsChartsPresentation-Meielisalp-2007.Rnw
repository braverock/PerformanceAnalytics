% $Header: /cvs/R/PerformanceAnalytics/inst/doc/PerformanceAnalyticsChartsPresentation-Meielisalp-2007.Rnw,v 1.3 2007-07-10 16:47:32 brian Exp $

\documentclass{beamer}

% need full path to find these .sty files outside of R environment
%\usepackage{Sweave}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
      ...
      \item
        ...
        install.packages(\char`\"{}PerformanceAnalytics\char`\"{})}
      \item
  ...
  using: \linebreak \texttt{> library(\char`\"{}PerformanceAnalytics\char`\"{})}.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \end{equation}
    \item
    Sortino Ratio --- improvement on Sharpe Ration utilizing downside semivariance as the measure of risk \\
    \begin{equation}
    ...
    \end{equation}
    \item
    Calmar and Sterling Ratios --- ratio of annualized return over the absolute value of the maximum drawdown
    \item
    Sortino's Upside Potential Ratio --- upside semdiviation from MAR over downside semidiviation from MAR
    \begin{equation}
