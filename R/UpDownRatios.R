...
#' @export
UpDownRatios <-
  function(Ra, Rb, method = c("Capture", "Number", "Percent"), side = c("Up", "Down"), geometric = TRUE) { # @author Peter Carl
    ...
    # geometric: utilize geometric chaining (TRUE) or simple/arithmetic chaining (FALSE).
    ...
    udr <- function(Ra, Rb, method, side, geometric) {
      ...
      if (method == "Capture" & side == "Up") {
        ...
        if (geometric) {
          # Annualization is mathematically identical in the ratio, so we can use Return.cumulative
          result <- Return.cumulative(UpRa) / Return.cumulative(UpRb)
        } else {
          # Bacon defines this as a ratio of means, but the ratio of sums and means is the same
      ...
      if (method == "Capture" & side == "Down") {
        ...
        if (geometric) {
          result <- Return.cumulative(DnRa) / Return.cumulative(DnRb)
        } else {
    ...
    result <- apply(pairs, 1, FUN = function(n, Ra, Rb, method, side, geometric) udr(Ra[, as.numeric(n[1])], Rb[, as.numeric(n[3])], method = n[4], side = n[2], geometric = geometric), Ra = Ra, Rb = Rb, geometric = geometric)
