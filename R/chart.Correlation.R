...
{ # @author R Development Core Team
  # @author modified by Peter Carl
    ...
    # Draw the chart
    if(histogram)
        pairs(x, gap=0, lower.panel=panel.smooth, pch=pch, upper.panel=panel.cor, diag.panel=hist.panel)
    else
        pairs(x, gap=0, lower.panel=panel.smooth, pch=pch, upper.panel=panel.cor) 
}
