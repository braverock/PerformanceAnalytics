...
#' @param method one of "modified","gaussian","historical", "kernel", "gpd", "lognormal", "montecarlo", see
#' Details.
...
#' @param p.tr probability threshold for \code{method="gpd"}, default 0.97
#' @param init initial parameter estimate for \code{method="gpd"}, default \code{c(1.00, 0.3)}
#' @param nsim the number of simulations used for \code{method="montecarlo"}, default 10000
...
#' density function.
#'
#' Other forms of parametric estimation utilize a different distribution for the
#' distribution of losses to better account for the possible fat-tailed nature of
#' downside risk. The now-archived package \code{VaR} by Talgat Daniyarov
#' contained methods for simulating and estimating lognormal and generalized
#' Pareto distributions. You may access the lognormal estimate using
#' \code{method="lognormal"}. A Generalized Pareto Distribution estimate is
#' available using \code{method="gpd"}. A Monte Carlo simulation estimate is
#' available using \code{method="montecarlo"}.
#'
...
ETL <- CVaR <- ES <- function(R = NULL, p = 0.95, ...,
                              ...
                              SE = FALSE, SE.control = NULL, p.tr = 0.97, init = c(1.00, 0.3), nsim = 10000) { # @author Brian G. Peterson
  ...
  if (!is.null(R)) {
    ...
    if (portfolio_method != "single") {
      ...
      if (method == "modified") {
        if (is.null(m3)) {
          m3 <- M3.MM(R, as.mat = FALSE, mu = mu)
        }
        if (is.null(m4)) {
          m4 <- M4.MM(R, as.mat = FALSE, mu = mu)
        }
      }
    }
  } else {
  ...
  if (SE && !(method %in% c("gpd", "lognormal"))) {
    ...
    for (mymethod in SE.control$se.method) {
      ses[[mymethod]] <- RPESE::EstimatorSE(R,
        estimator.fun = "ES", se.method = mymethod,
  ...
  switch(portfolio_method,
    single = {
      if (is.null(weights)) {
        ...
        switch(method,
          modified = {
            if (operational) {
              rES <- operES.CornishFisher(R = R, p = p)
            } else {
              rES <- ES.CornishFisher(R = R, p = p)
            }
          }, # mu=mu, sigma=sigma, skew=skew, exkurt=exkurt))},)
          gaussian = {
            rES <- ES.Gaussian(R = R, p = p)
          },
          historical = {
          ...
          montecarlo = {
            rES <- ES.montecarlo(R = R, p = p, nsim = nsim, ...)
          }
        ...
        switch(method,
          modified = {
            rES <- mES.MM(w = weights, mu = mu, sigma = sigma, M3 = m3, M4 = m4, p = p)
          },
          gaussian = {
            rES <- GES.MM(w = weights, mu = mu, sigma = sigma, p = p)
          },
          historical = {
          ...
          montecarlo = {
            rES <- as.matrix(ES.montecarlo(R = R, p = p, nsim = nsim, ...)) %*% weights
          }
    ...
    component = {
      ...
      switch(method,
        modified = {
          if (operational) {
            return(operES.CornishFisher.portfolio(p, weights, mu, sigma, m3, m4))
          } else {
            return(ES.CornishFisher.portfolio(p, weights, mu, sigma, m3, m4))
          }
        },
        gaussian = {
          return(ES.Gaussian.portfolio(p, weights, mu, sigma))
        },
        historical = {
          return(ES.historical.portfolio(R, p, weights))
        },
        kernel = {
          return(ES.kernel.portfolio(R = R, p = p, w = weights))
        },
        gpd = {
          stop("no component method defined for gpd ES")
        },
        lognormal = {
          stop("no component method defined for lognormal ES")
        },
        montecarlo = {
          return(ES.montecarlo.portfolio(R, p, weights, nsim = nsim, ...))
        }
      )
    }, # end component portfolio switch
    marginal = {
      stop(paste("no marginal method defined for", method, "ES"))
