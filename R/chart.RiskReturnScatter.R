...
chart.RiskReturnScatter <-
  function(R, Rf = 0, main = "Annualized Return and Risk", add.names = TRUE, xlab = "Annualized Risk", ylab = "Annualized Return", method = "calc", geometric = TRUE, scale = NA, add.sharpe = c(1, 2, 3), add.boxplots = FALSE, colorset = 1, symbolset = 1, element.color = "darkgray", legend.loc = NULL, xlim = NULL, ylim = NULL, cex.legend = 1, cex.axis = 0.8, cex.main = 1, cex.lab = 1, na.fill = FALSE, ...) { # @author Peter Carl
    ...
    if (method == "calc") {
      x <- checkData(R, method = "matrix") # Coerce to xts natively via checkData's parsing, enforcing matrix format structure internally
      if (is.numeric(na.fill)) {
        x[is.na(x)] <- na.fill
      }
    } else {
      x <- R
    ...
    if (method == "calc") {
      ...
      comparison <- t(table.AnnualizedReturns(x[, columns:1, drop = FALSE], Rf = Rf, geometric = geometric, scale = scale))
    ...
    if (is.null(xlim[1])) {
      max_risk <- suppressWarnings(max(risk, na.rm = TRUE))
      if (is.infinite(max_risk)) max_risk <- 0.1
      xlim <- c(0, max_risk + 0.02)
    }
    if (is.null(ylim[1])) {
      min_ret <- suppressWarnings(min(c(0, returns), na.rm = TRUE))
      max_ret <- suppressWarnings(max(returns, na.rm = TRUE))
      if (is.infinite(min_ret)) min_ret <- 0
      if (is.infinite(max_ret)) max_ret <- 0.1
      ylim <- c(min_ret, max_ret + 0.02)
    }

    if (add.boxplots) {
      original.layout <- par(no.readonly = TRUE)
      layout(
        matrix(c(2, 1, 0, 3), 2, 2, byrow = TRUE),
        c(1, 6), c(4, 1),
      )
      par(mar = c(1, 1, 5, 2))
    }
    ...
    if (add.names) {
      text(x = risk, y = returns, labels = rnames, pos = 4, cex = 0.8, col = colorset[columns:1])
    } # adj = -0.1
