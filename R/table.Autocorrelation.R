...
#' @param max.lag the maximum autocorrelation lag to include in the table
...
table.Autocorrelation <-
  ...
  {# @author Peter Carl
    ...
    for(column in 1:columns) { # for each asset passed in as R
      ...
      acflag = acf(y,plot=FALSE,lag.max=max.lag)[[1]][2:(max.lag+1)]
      LjungBox =  Box.test(y,type="Ljung-Box",lag=6)
      values = c(acflag, LjungBox$p.value)
    ...
    rownames(result.df) = c(paste(rep("rho", max.lag), 1:max.lag), paste0("Q(", max.lag, ") p-value"))
