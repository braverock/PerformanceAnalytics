...
CDaR.alpha <- function(R, Rm, p = 0.95, weights = NULL, geometric = TRUE, type = NULL, ...) {
  ...
  if (is.vector(R) || ncol(R) == 1) {
    ...
    if (is.null(weights)) {
      beta <- vector("numeric", length = ncol(R))
      for (i in 1:ncol(R)) {
        beta[i] <- CDaR.beta(R[, i, drop = FALSE], Rm = Rm, p = p, geometric = geometric, ... = ...)
      }
    } else {
  ...
  if (is.null(weights) && !is.vector(R) && ncol(R) > 1) {
    R_expected_annualized <- (1 + apply(R, 2, mean))^12 - 1
  } else {
