...
AverageDrawdown <-
  function(R, ...) {
    ...
    ad <- function(R) {
      R <- na.omit(R)
      Dj <- findDrawdowns(R)$return
      d <- length(Dj[Dj < 0])
      result <- abs(sum(Dj[Dj < 0] / d))
      return(result)
    }

    result <- sapply(1:NCOL(R), function(i) ad(R[, i, drop = FALSE]))
    dim(result) <- c(1, NCOL(R))
    colnames(result) <- colnames(R)
...
AverageRecovery <-
  function(R, ...) {
    ...
    result <- sapply(1:NCOL(R), function(i) ar(R[, i, drop = FALSE]))
    dim(result) <- c(1, NCOL(R))
    colnames(result) <- colnames(R)
...
AverageLength <-
  function(R, ...) {
    ...
    result <- sapply(1:NCOL(R), function(i) al(R[, i, drop = FALSE]))
    dim(result) <- c(1, NCOL(R))
    colnames(result) <- colnames(R)
