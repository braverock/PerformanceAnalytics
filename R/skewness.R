...
#' \deqn{Skewness(fisher) = \frac{\sqrt{n*(n-1)}}{n-2}*\frac{\sum^{n}_{i=1}(x_i - \overline{x})^3/n}{(\sum^{n}_{i=1}(x_i - \overline{x})^2/n)^{3/2}}}{Skewness(fisher)((sqrt(n*(n-1))/(n-2))*(sum((x-mean(x))^3)/n))/((sum((x-mean(x))^2)/n)^(3/2))}
#'
