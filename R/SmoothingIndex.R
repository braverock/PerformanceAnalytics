...
#' procedure to observed de-meaned returns...". To comply with this, we explicitly
#' subtract the mean from the returns prior to the \code{arima} fit and set
#' \code{include.mean=FALSE} to avoid intercept absorption into the MA terms.
#'
...
#' @export
SmoothingIndex <-
  function(R, neg.thetas = FALSE, MAorder = 2, verbose = FALSE, ...) { # @author Peter Carl
    ...
    if (is.vector(R)) {
      ...
      # to observed de-meaned returns...", so we explicitly demean the returns
      # and set include.mean to FALSE.
      ...
      R_demean <- R - mean(R, na.rm = TRUE)
      MA2 <- arima(R_demean, order = c(0, 0, MAorder), method = "ML", transform.pars = TRUE, include.mean = FALSE)
