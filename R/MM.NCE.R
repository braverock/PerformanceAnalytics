...
NCE_obj <- function(theta, p, k, W, m2, m3, m4, include.mom, B, epsvar, fskew, epsskew, fkurt, epskurt,
  ...
  if (include.mom[2]) {
    grad3 <- .Call("mod3grad", p, k, as.numeric(B), Binclude, epsvarinclude,
      fskew, fskewinclude, epsskewinclude,
...
MM.NCE <- function(R, as.mat = TRUE, ...) {
    ...
    if (is.list(W)) {
      if ("W2" %in% names(W)) {
        ...
        if (is.null(alpha)) {
          ...
          if ("optscontrol" %in% nW) {
            optscontrol_B <- W$optscontrol
          } else {
            optscontrol_B <- list(
              algorithm = "NLOPT_LD_MMA", xtol_rel = 1e-5, ftol_rel = 1e-5,
              ftol_abs = 1e-5, maxeval = 5000, print_level = 0, check_derivatives = FALSE
            )
          }
          x0_B <- NCEinitMCA(x, k, include.mom = include.mom)
          NC_temp <- MM.NCE(x,
            as.mat = FALSE, k = k, x0 = x0_B, include.mom = include.mom,
  ...
  if (hasArg(optscontrol)) {
    ...
    optscontrol <- list(
      algorithm = "NLOPT_LD_MMA", xtol_rel = 1e-05,
  ...
  if (optimize_method == "nloptr") {
    ...
    if (include.ineq) {
      sol <- nloptr::nloptr(
        x0 = x0, eval_f = NCE_obj, eval_g_ineq = NCE_obj_ineq,
      ...
      sol <- nloptr::nloptr(
        x0 = x0, eval_f = NCE_obj, lb = lowerbound, ub = upperbound,
    ...
    sol <- rgenoud::genoud(
      fn = NCE_objDE, nvars = nvars, max = FALSE, pop.size = pop.size,
    ...
    if (include.ineq) {
      sol <- nloptr::nloptr(
        x0 = sol$par, eval_f = NCE_obj, eval_g_ineq = NCE_obj_ineq,
        lb = lowerbound, ub = upperbound, opts = optscontrol,
        p = p, k = k, W = W, m2 = m2, m3 = m3, m4 = m4,
        include.mom = include.mom, B = B, epsvar = epsvar, fskew = fskew,
        epsskew = epsskew, fkurt = fkurt, epskurt = epskurt,
      ...
      sol <- nloptr::nloptr(
        x0 = sol$par, eval_f = NCE_obj, lb = lowerbound, ub = upperbound,
        opts = optscontrol, p = p, k = k, W = W, m2 = m2, m3 = m3, m4 = m4,
        include.mom = include.mom, B = B, epsvar = epsvar, fskew = fskew,
        epsskew = epsskew, fkurt = fkurt, epskurt = epskurt,
        W2 = W2, W3 = W3, W4 = W4
  ...
  if (model.par) {
    result$model.par <- list(
      "B" = B, "epsvar" = epsvar, "fskew" = fskew,
      "epsskew" = epsskew, "fkurt" = fkurt, "epskurt" = epskurt
    )
  }
...
NCEconstructW <- function(X, Wid = "RidgeD", alpha = 0.1, include.mom = c(TRUE, TRUE, TRUE)) {
  ...
  if (length(Wid) == 1 && Wid == "Id") {
    ...
    Xi_obs <- .Call("NCEAcov", as.numeric(m11), p, n, as.numeric(Xc),
      include.mom[1], include.mom[2], include.mom[3],
      PACKAGE = "PerformanceAnalytics"
    )
    if (length(Wid) == 1) {
      if (Wid == "D") {
...
bootstrap_alpha_Ridge <- function(X, nb, alphavec, k, x0, optscontrol,
  ...
  for (ii in 1:nb) {
    ...
    for (jj in 1:length(alphavec)) {
      ...
      NCest <- MM.NCE(Xboot,
        include.mom = include.mom, as.mat = FALSE,
