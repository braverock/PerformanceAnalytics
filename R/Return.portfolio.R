...
#' weights, as decimal percentages. When passed as a time series, weights are
#' treated as end-of-period targets that take effect at the start of the subsequent period.
#' See Details below.
...
Return.portfolio <- Return.rebalancing <- function(R,
  ...
  if (any(is.na(R))) {
    if (!full_investment) warning("NA's detected: filling NA's with zeros")
    # R <- zerofill(R)
    R[is.na(R)] <- 0
  }
  ...
  if (time_unit == "quarter") {
    start_date <- as.yearqtr(seq(as.Date(index(R)[1]), length.out = 2, by = paste("-3", "month"))[2])
  } else {
    start_date <- seq(as.Date(index(R)[1]), length.out = 2, by = paste("-1", time_unit))[2]
  }
  if (is.null(weights)) {
  ...
  if (full_investment) {
    state_diff <- diff(R_isna) != 0
    change_idx <- which(rowSums(state_diff, na.rm = TRUE) > 0)

    if (length(change_idx) > 0) {
      # Stamp the new weights on the day BEFORE the state change so they take effect immediately
      change_dates <- index(R)[change_idx]

      first_weight_date <- as.Date(index(weights[1, ]))
      change_dates <- change_dates[as.Date(change_dates) >= first_weight_date]

      if (length(change_dates) > 0) {
        all_dates <- sort(unique(c(index(weights), change_dates)))

        # Merge to align existing weights and new rebalance boundaries, then fill forward
        weights <- merge(weights, xts(order.by = all_dates))
        weights <- zoo::na.locf(weights, na.rm = FALSE)

        for (i in 1:nrow(weights)) {
          d <- index(weights[i, ])

          # Find the exact period the weights will apply to (the first date > d)
          next_d_idx <- which(index(R) > d)[1]

          if (!is.na(next_d_idx)) {
            avail <- !R_isna[next_d_idx, ]
            w <- coredata(weights[i, ])
            w[!avail] <- 0
  ...
  if (geometric) {
    out <- Return.portfolio.geometric(
      R = R,
      weights = weights,
    ...
    out <- Return.portfolio.arithmetic(
      R = R,
      weights = weights,
      wealth.index = wealth.index,
      contribution = contribution,
