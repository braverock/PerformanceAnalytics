...
UpsidePotentialRatio <-
  function(R, MAR = 0, method = c("subset", "full")) { # @author Brian G. Peterson
    ...
    method <- method[1]
    R <- checkData(R)
    attr(R, ".RECLASS") <- FALSE

    upr <- function(R, MAR, method) {
      if (inherits(R, "zoo") && !is.timeBased(index(R))) {
        R <- as.vector(coredata(R))
      }
      R <- na.omit(R)

      R_excess <- Return.excess(R, MAR)
      excess <- R_excess[R_excess > 0]
    ...
    result <- sapply(1:ncol(R), function(i) upr(R[, i, drop = FALSE], MAR = MAR, method = method))
