...
chart.SnailTrail <-
  function(R, Rf = 0, main = "Annualized Return and Risk", add.names = c("all", "lastonly", "firstandlast", "none"), xlab = "Annualized Risk", ylab = "Annualized Return", add.sharpe = c(1, 2, 3), colorset = 1:12, symbolset = 16, legend.loc = NULL, xlim = NULL, ylim = NULL, width = 12, stepsize = 12, lty = 1, lwd = 2, cex.axis = 0.8, cex.main = 1, cex.lab = 1, cex.text = 0.8, cex.legend = 0.8, element.color = "darkgray", ...) { # @author Peter Carl
    ...
    start.row <- 1
    start.index <- 0
    while (start.row <= nrow(x) && is.na(x[start.row, 1])) {
      start.row <- start.row + 1
    }

    if (start.row > nrow(x) || length.column.one == 0) {
      warning("The first column is entirely NA or too short. SnailTrail requires valid data in the first column.")
      return(invisible())
    }
    ...
    for (column in 1:columns) {
      ...
      if (nrow(y) < width) {
        returns.column <- xts::xts(numeric(0), order.by = as.Date(character(0)))
        risk.column <- xts::xts(numeric(0), order.by = as.Date(character(0)))
      } else {
        y <- as.zoo(y)
    ...
    if (maxrows == 0) {
      warning("Not enough data to calculate rolling metrics for any column.")
      return(invisible())
    }
    ...
    for (column in columns:1) {
      ...
      for (i in 1:length(returns[, column])) {
        color_idx <- max(1, maxrows - i + 1)
        points(risk[i, column], returns[i, column], pch = symbolset[column], col = colortrail[color_idx])
      }
      # Attach the points with lines
      for (i in 2:length(returns[, column])) {
        color_idx <- max(1, maxrows - i + 1)
        segments(risk[i, column], returns[i, column], risk[i - 1, column], returns[i - 1, column], col = colortrail[color_idx], lty = lty, lwd = lwd)
      }
