...
Return.convert <- function(
  ...
  if (is.null(seedValue)) {
    seedValuePassed <- FALSE
  } else {
    seedValuePassed <- TRUE
  }
  ...
  if (length(destinationType) > 1) {
    stop("Must select a single destinationType, one of 'discrete','log','difference' or 'level'")
  }
  # End Error-check
  ...
  coredata_content <- attributes(R)$coredata_content
  if (length(coredata_content) == 0) {
    coredata_content <- "discreteReturn"
  }

  switch(coredata_content,
    discreteReturn = { # done
      switch(destinationType,
        discrete = {
          warning(sameTypeWarning)
          return(R)
        },
        log = {
          result <- log(1 + R)
          attributes(result)$coredata_content <- "logReturn"
        },
        difference = {
          result <- R * xts::lag.xts(Level.calculate(R = R, seedValue = seedValue, initial = initial))
          attributes(result)$coredata_content <- "difference"
          if (!seedValuePassed) {
            warning(noSeedValueWarning)
          }
        },
        level = {
          warning(usingLevelCalculateWarning)
          result <- Level.calculate(R = R, seedValue = seedValue, initial = initial)
        },
        {
    ...
    logReturn = { # done
      switch(destinationType,
        discrete = {
          result <- exp(R) - 1
          attributes(result)$coredata_content <- "discreteReturn"
        },
        log = {
          warning(sameTypeWarning)
          return(R)
        ...
        difference = {
          result <- (exp(R) - 1) * xts::lag.xts(Level.calculate(R = R, seedValue = seedValue, initial = initial))
          attributes(result)$coredata_content <- "difference"
          if (!seedValuePassed) {
            warning(noSeedValueWarning)
          }
    ...
    difference = { # done
      switch(destinationType,
        ...
        log = {
          result <- log(R / xts::lag.xts(Level.calculate(R = R, seedValue = seedValue, initial = initial)) + 1)
          attributes(result)$coredata_content <- "logReturn"
        },
        difference = {
          warning(sameTypeWarning)
    ...
    level = {
      switch(destinationType,
        discrete = {
          result <- Return.calculate(prices = R, method = "discrete")
        },
        log = {
          result <- Return.calculate(prices = R, method = "log")
        },
        difference = {
          result <- Return.calculate(prices = R, method = "difference")
          if (!seedValuePassed) {
            warning(noSeedValueWarning)
          }
        },
