...
checkSeedValue <- function(R, seedValue) {
  ...
  if (length(attributes(R)$coredata_content) == 0) {
    ...
    # Actually, R is passed by value, so modifying attributes(R) here doesn't change it in the caller.
    # So we'll just check it locally to assign the seedValue correctly.
    R_content <- "discreteReturn"
  } else {
    R_content <- attributes(R)$coredata_content
  }

  # Error check initial value before starting, so if no seedValue passed, set to 1 if it's not 'difference'
  if (is.null(seedValue) & (R_content == "difference")) {
    stop("When calculating levels using 'difference'(s), a seedValue must be provided")
  } else {
    if (is.null(seedValue) & !(R_content == "difference")) {
      seedValue <- 1
    }
  ...
  if (class(seedValue)[1] == "xts") {
    seedValue <- as.vector(zoo::coredata(seedValue))
  }
