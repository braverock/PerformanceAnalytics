...
lpm <- function(R, n = 2, threshold = 0, about_mean = FALSE,
  ...
  if (is.null(threshold) || is.na(threshold) || !is.numeric(threshold)) {
    stop("threshold must be a valid numeric value.")
  }
  if (is.null(n) || is.na(n) || !is.numeric(n)) {
    stop("n must be a valid numeric value.")
  }
  ...
  for (column in seq_len(columns)) {
    ...
    x <- as.numeric(coredata(R1))

    # Use logical masks on numeric array, avoiding slow xts subsetting
    mask <- !is.na(x) & (x < threshold)
    x_cond <- x[mask]
    nb <- length(x_cond)

    if (nb == 0) {
      warning(paste("No observations below the threshold for column", columnnames[column], "- returning NA"))
      LPM <- NA_real_
    } else {
      if (about_mean == TRUE) {
  ...
  if (SE) {
    ...
    for (mymethod in SE.control$se.method) {
      ses[[mymethod]] <- RPESE::EstimatorSE(R,
        estimator.fun = "LPM", se.method = mymethod,
