...
#' @param p.tr for \code{method="gpd"}, the threshold probability for GPD fit, default 0.97
#' @param init for \code{method="gpd"}, initial values for the Nelder-Mead optimization, default \code{c(1.00, 0.3)}
#' @param nsim for \code{method="montecarlo"}, number of simulations, default 10000
...
#' \code{VaR} by Talgat Daniyarov contained methods for simulating and estimating lognormal and
...
#' on potentially fat-tailed distributions. You may access the lognormal
#' estimate using \code{method="lognormal"}. A Generalized Pareto Distribution
#' estimate is available using \code{method="gpd"}. A Monte Carlo simulation
#' estimate is available using \code{method="montecarlo"}. The \code{\link{VaR.backtest}}
#' function may be used to apply a binomial proportion of failures test to any
#' of these VaR estimates to evaluate their ex-post accuracy.
#'
...
#' \code{\link{SharpeRatio}} for more information.
#'
...
#' @export
VaR <-
  function(R = NULL, p = 0.95, ..., method = c("modified", "gaussian", "historical", "kernel", "gpd", "lognormal", "montecarlo"),
    ...
    if (SE && !(method %in% c("gpd", "lognormal"))) {
      ...
      for (mymethod in SE.control$se.method) {
        ses[[mymethod]] <- RPESE::EstimatorSE(R,
          estimator.fun = "VaR", se.method = mymethod,
    ...
    switch(portfolio_method,
      single = {
        if (is.null(weights)) {
          switch(method,
            ...
            historical = {
              rVaR <- -1 * t(apply(R, 2, quantile, probs = 1 - p, na.rm = TRUE))
            },
            kernel = {
            ...
            montecarlo = {
              rVaR <- VaR.montecarlo(R = R, p = p, nsim = nsim, ...)
            }
          ...
          switch(method,
            ...
            montecarlo = {
              rVaR <- as.matrix(VaR.montecarlo(R = R, p = p, nsim = nsim, ...)) * weights
            }
      ...
      component = {
        ...
        switch(method,
          ...
          gpd = {
            stop("no component method defined for gpd VaR")
          },
          lognormal = {
            stop("no component method defined for lognormal VaR")
          },
          montecarlo = {
            return(VaR.montecarlo.portfolio(R, p, weights, nsim = nsim, ...))
          }
        )
      }, # end component portfolio switch
      marginal = {
        if (method %in% c("gpd", "lognormal")) {
          stop(paste("no marginal method defined for", method, "VaR"))
        }
