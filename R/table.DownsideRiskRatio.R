...
table.DownsideRiskRatio <-
  function(R, MAR = 0, scale = NA, digits = 4) {
    ...
    if (is.na(scale)) {
      ...
      label_downside <- paste(freq$scale, "downside risk")
    } else {
      label_downside <- "Downside risk"
    }
    ...
    for (column in 1:columns) {
      ...
      znames <- c(label_downside, "Annualised downside risk", "Downside potential", "Omega", "Sortino ratio", "Upside potential", "Upside potential ratio", "Omega-sharpe ratio")
      if (column == 1) {
