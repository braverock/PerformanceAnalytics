...
#' @export
table.CaptureRatios <-
  function(Ra, Rb, digits = 4, geometric = TRUE) { # @author Peter Carl
    ...
    for (column.a in 1:columns.a) { # for each asset passed in as R
      ...
      UpCapture <- UpDownRatios(Ra = merged.assets[, 1], Rb = merged.assets[, 2], method = "Capture", side = "Up", geometric = geometric)
      DnCapture <- UpDownRatios(Ra = merged.assets[, 1], Rb = merged.assets[, 2], method = "Capture", side = "Down", geometric = geometric)
