...
table.Distributions <-
  function(R, scale = NA, digits = 4) {
    ...
    if (is.na(scale)) {
      ...
      label_stddev <- paste(freq$scale, "Std Dev")
    } else {
      label_stddev <- "Std Dev"
    }
    ...
    for (column in 1:columns) {
      ...
      znames <- c(label_stddev, "Skewness", "Kurtosis", "Excess kurtosis", "Sample skewness", "Sample excess kurtosis")
