...
This is not an exhaustive list and in the last subsection we refer the reader to other possibilities, such as EWMA estimation (also available in \code{\link{PerformanceAnalytics}}) or the GO-GARCH model in \code{\link{rmgarch}}.
...
summ_moms <- matrix(NA, nrow = 15, ncol = 2)
...
<<ShrinkIndep,echo=T>>=
# target "Indep"
m3 <- M3.shrink(edhec, c(T, F, F, F))$M3sh
m4 <- M4.shrink(edhec, c(T, F, F, F))$M4sh

# target "IndepId"
m3 <- M3.shrink(edhec, c(F, T, F, F))$M3sh
m4 <- M4.shrink(edhec, c(F, T, F, F))$M4sh

# target "1-factor"
m3 <- M3.shrink(edhec, c(F, F, T, F), f)$M3sh
m4 <- M4.shrink(edhec, c(F, F, T, F), f)$M4sh

# target "constant correlation"
m3 <- M3.shrink(edhec, c(F, F, F, T))$M3sh
m4 <- M4.shrink(edhec, c(F, F, F, T))$M4sh

# target "latent 1-factor model of Simaan (1993)"
m3 <- M3.shrink(edhec, c(F, F, F, F, T, F))$M3sh

# target "central-symmetry"
m3 <- M3.shrink(edhec, c(F, F, F, F, F, T))$M3sh
@
...
m3 <- M3.shrink(edhec, c(F, F, F, T), f)$M3sh
m4 <- M4.shrink(edhec, c(F, F, F, T), f)$M4sh
m3port <- PerformanceAnalytics:::portm3(w, m3)
m4port <- PerformanceAnalytics:::portm4(w, m4)
...
\subsection{Unbiased estimatino of the MSE loss function}
...
<<ShrinkMTunb,echo=T>>=
m3 <- M3.shrink(edhec, c(T, T, F, F, F, T), unbiasedMSE = T)$M3sh
@
...
<<VaRestimates,echo=F>>=
rm95 <- matrix(NA, nrow = 11, ncol = 2)
VaRcomp <- matrix(NA, nrow = 11, ncol = 13)
EScomp <- matrix(NA, nrow = 11, ncol = 13)
...
temp <- VaR(p = p, method = "modified", portfolio_method = "component", 
            weights = w, mu = m, sigma = sigma, m3 = M3.shrink(edhec, c(T, F, F, F), as.mat = F)$M3sh, 
            m4 = M4.shrink(edhec, c(T, F, F, F), as.mat = F)$M4sh)
rm95[6, 1] <- temp$MVaR
VaRcomp[6,] <- temp$pct_contrib_MVaR

temp <- VaR(p = p, method = "modified", portfolio_method = "component", 
            weights = w, mu = m, sigma = sigma, m3 = M3.shrink(edhec, c(F, T, F, F), as.mat = F)$M3sh, 
            m4 = M4.shrink(edhec, c(F, T, F, F), as.mat = F)$M4sh)
rm95[7, 1] <- temp$MVaR
VaRcomp[7,] <- temp$pct_contrib_MVaR

temp <- VaR(p = p, method = "modified", portfolio_method = "component", 
            weights = w, mu = m, sigma = sigma, m3 = M3.shrink(edhec, c(F, F, T, F), f = f, as.mat = F)$M3sh, 
            m4 = M4.shrink(edhec, c(F, F, T, F), f= f, as.mat = F)$M4sh)
rm95[8, 1] <- temp$MVaR
VaRcomp[8,] <- temp$pct_contrib_MVaR

temp <- VaR(p = p, method = "modified", portfolio_method = "component", 
            weights = w, mu = m, sigma = sigma, m3 = M3.shrink(edhec, c(F, F, F, T), as.mat = F)$M3sh, 
            m4 = M4.shrink(edhec, c(F, F, F, T), as.mat = F)$M4sh)
rm95[9, 1] <- temp$MVaR
VaRcomp[9,] <- temp$pct_contrib_MVaR

temp <- VaR(p = p, method = "modified", portfolio_method = "component", 
            weights = w, mu = m, sigma = sigma, m3 = M3.shrink(edhec, c(T, F, T, T), f = f, as.mat = F)$M3sh, 
            m4 = M4.shrink(edhec, c(T, F, T, T), f = f, as.mat = F)$M4sh)
rm95[10, 1] <- temp$MVaR
