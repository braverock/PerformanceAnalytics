...
test_that("chart.TimeSeries gracefully propagates ylog=TRUE through all interactive engines without failure", {
  skip_on_cran()
  data(edhec)
  R <- edhec[1:50, 1, drop = FALSE]

  if (requireNamespace("plotly", quietly = TRUE)) {
    expect_error(suppressWarnings(chart.TimeSeries(R, plot.engine = "plotly", ylog = TRUE)), NA)
  }

  if (requireNamespace("dygraphs", quietly = TRUE)) {
    expect_error(suppressWarnings(chart.TimeSeries(R, plot.engine = "dygraph", ylog = TRUE)), NA)
  }

  if (requireNamespace("googleVis", quietly = TRUE)) {
    expect_error(suppressWarnings(chart.TimeSeries(R, plot.engine = "googlevis", ylog = TRUE)), NA)
  }
})
