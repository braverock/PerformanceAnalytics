...
test_that("SharpeRatio uses excess-return standard deviation (fixes #142)", {
  ...
  old_sr_scalar <- Return.annualized(Return.excess(R, Rf_scalar), geometric = FALSE) / StdDev.annualized(R)
  new_sr_scalar <- Return.annualized(Return.excess(R, Rf_scalar), geometric = FALSE) / StdDev.annualized(Return.excess(R, Rf_scalar))
  ...
  old_sr_vec <- Return.annualized(Return.excess(R, Rf_vec), geometric = FALSE) / StdDev.annualized(R)
  new_sr_vec <- Return.annualized(Return.excess(R, Rf_vec), geometric = FALSE) / StdDev.annualized(Return.excess(R, Rf_vec))
