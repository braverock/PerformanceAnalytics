...
test_that("Return.portfolio matches legacy tinytest expectations", {
  ...
  R_na <- R_monthly
  R_na[1,1] <- NA
  # It seems the warning is actually being caught, but let's be safe.
  out_na <- suppressWarnings(Return.portfolio(R_na, weights = c(0.5, 0.5)))
  ...
  expect_s3_class(out_vec_reb, "xts")
  ...
  expect_warning(out_rets <- Return.portfolio(R_monthly, weights=w_not1, geometric=TRUE), "sum up to 1")
...
test_that("Return.portfolio.geometric matches legacy expectations", {
  # Mock objects
  ...
  returns <- suppressWarnings(PerformanceAnalytics:::Return.portfolio.geometric(test_returns, 
                                                               test_weights, 
                                                               wealth.index =TRUE, 
                                                               verbose =TRUE))
  
  returns_val <- suppressWarnings(PerformanceAnalytics:::Return.portfolio.geometric(test_returns, 
                                                                   test_weights, 
                                                                   wealth.index =TRUE, 
                                                                   value =100000, 
                                                                   verbose =TRUE))
  
  expect_equal(as.vector(returns$returns), 
               expected_returns, 
               tolerance = epsilon)
  ...
  returns_main <- suppressWarnings(PerformanceAnalytics:::Return.portfolio(test_returns, 
                                                          test_weights, 
                                                          wealth.index =TRUE, 
                                                          verbose =TRUE))
  
  returns_main_val <- suppressWarnings(PerformanceAnalytics:::Return.portfolio(test_returns, 
                                                              test_weights, 
                                                              wealth.index =TRUE, 
                                                              value =100000, 
                                                              verbose =TRUE))
  
  expect_equal(as.vector(returns_main$returns), 
               expected_returns, 
               tolerance = epsilon)
